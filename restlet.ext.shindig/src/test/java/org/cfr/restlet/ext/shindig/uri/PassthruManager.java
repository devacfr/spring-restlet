 ...
 * under the License.
 */
package org.cfr.restlet.ext.shindig.uri;
...
public class PassthruManager implements ProxyUriManager {
    ...
    private Uri getUri(Uri src) {
        ...
        return new UriBuilder().setScheme("http").setAuthority(proxyHost).setPath(proxyPath).addQueryParameter(Param.URL.getKey(), src.toString())
        .toUri();
    }

    public ProxyUri process(Uri uri) throws GadgetException {
        String proxied = uri.getQueryParameter(Param.URL.getKey());
        return new ProxyUri(expectStatus, proxied != null ? Uri.parse(proxied) : null, uri);
    }
